...
​		**daw**:	  删除(剪切)一个词(不管光标是否在词首)

​		**das**:		删除(剪切)一句(以英文.及空格为断句条件，如不满足断句条件将继续往下直至行末，存在误删机率)
...
​		**yas**:		复制一句(不用管光标位置及换行，根据标点符号**.**及空格断句，如果.后没有空格或使用中文句号则断句条件不成立)
...
​			**caw**:		删除光标所在那个词，并进入insert模式(不用管光标是否在词首，空格或标点为分词条件，标点也算一个词)

​			**cas**:		删除一句，并进入insert模式(以英文.及空格断句，如果不符合这个条件就继续往后找断句直到行末，存在误删除的机率)
